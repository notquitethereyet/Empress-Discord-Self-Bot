    ...
    const [cmd, ...args] = message.content.trim().substring(PREFIX.length).split(/\s+/);
        switch(cmd){
            case '8ball':{
                ...
                case 'allmyhomies':{
                    if(imgflip_pass===""||imgflip_username==="") {
                        message.delete()
                        await open('https://imgflip.com/signup');
                        return console.log(clc.red("please verify imgflip credentials in config"));
                    }
                    console.log(clc.blueBright(`${cmd} command used`))
                    text = args.join(' ')
                    message.delete();
                    ...
                    x = imgflipper.generateMeme(216523697, "Fuck "+text, "All my homies hate "+text, cb);
                        ...
                        console.log(`${cmd}: Please check if you have set the imgflip.com credentials in config`)}
                ...
                case 'eval':{
                    ...
                    try {
                        ...
                        return message.channel.send("", { embed: new Discord.RichEmbed().setTitle("Results").setColor("#FF0000").setDescription(":desktop: **Input**: ```" + args.join("") + "```:eyes: **Error**: ```" + err + "```").setThumbnail('https://cdn.discordapp.com/attachments/767727403555356673/805083294496063498/18358674_1293395147381813_176585671498804251_o.jpg').setFooter("Empress") }).then(m=>{
                            setTimeout(function(){m.delete();},ET);
                        });
                    }
                    message.channel.send("", { embed: new Discord.RichEmbed().setTitle("Results").setColor("#46FF00").setDescription(":desktop: **Input**: ```" + args.join("") + "```:white_check_mark: **Output**: ```" + res + "```").setThumbnail('https://cdn.discordapp.com/attachments/767727403555356673/805083294496063498/18358674_1293395147381813_176585671498804251_o.jpg').setFooter("Empress") }).then(m=>{
                        setTimeout(function(){m.delete();},ET);
                    });
                    message.delete();
                    break;
